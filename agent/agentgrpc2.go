...
func main() {
	...
	for range ticker.C {
		...
		if usedSlots < 2 {
			...
			if err != nil {
				if err == sql.ErrNoRows {
					...
					connectResponse, err := client.Connect(context.Background(), &pb.ConnectRequest{Id: "agentgrpc2"})
					if err != nil {
		...
		_, err = db.Exec("UPDATE calc SET stat = 'в работе', agent = 'agentgrpc2' WHERE id = ?", id)
		if err != nil {
			log.Fatal(err)
		}
